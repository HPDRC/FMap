...
bool CFormatPaintTool::DrawShape(CShape* pShape, ShapeUtility::ShapeType shapeType, CString strColor, int nPenWidth)
{
	...
	case ShapeUtility::point:
		{
			...
			if( (!pShape->m_aProperties[2].IsEmpty()) || (!pShape->m_aProperties[8].IsEmpty()/*for tfgauges*/) || (pShape->m_aProperties[2].IsEmpty()))
			{
				...
				if( !pShape->m_aProperties[8].IsEmpty() ) /*for tfgauges*/
				{
					file = "rtstream.raw";
				}
			...
			{
				CPen iconPen;
				iconPen.CreatePen( PS_SOLID, this->niconsize, RGB(0, 255, 0) );
				pDC->SelectObject(&iconPen);
	...
	case ShapeUtility::arc:
		{ 
			...
			/*if(colorInfo.line != "")
			{*/
			arcColorManager.SetArcColor( this->colorInfo );
			...
			//Gdiplus::Color color(0xff, 0xff, 0xff, 0x00);
			////color.SetFromCOLORREF(arcColor);
			//Gdiplus::Pen pen(color);

			//for( iVertex = 0; iVertex < nCount-1; iVertex++ )
			//{
			//	graphics.DrawLine(&pen, round(pShape->m_aPointArray[iVertex].dX), 
			//		round(pShape->m_aPointArray[iVertex].dY), 
			//		round(pShape->m_aPointArray[iVertex+1].dX), 
			//		round(pShape->m_aPointArray[iVertex+1].dY) );
			//}
	...
	case ShapeUtility::polygon:
		{
			...
			{
				// area color from dbf or database column
				colorStr = pShape->m_aProperties[3];
			}	
				...
				("" != colorInfo.area) )
			{
				...
				else
				{
					...
					//logBrush.lbColor = RGB(255,255,0);//RGB(255,255,0);//
				}
				...
				int nPartsPaint = (int)pShape->m_aPartsStartingIndex.GetCount();
				...
				for(int i = 0; i < nPartsPaint; i++ )
				{
					...
					if ( i != (nPartsPaint - 1) )
					{
						nPartEndIndexPaint = (int)pShape->m_aPartsStartingIndex[i+1];														
					}
					else
					{	
						nPartEndIndexPaint = (int)pShape->m_aPointArray.GetCount();
					}	 
					//if(nParts > 1)
					//{
					...
					{	
						double x = pShape->m_aPointArray[j].dX;
						double y = pShape->m_aPointArray[j].dY;
			...
			//draw board
			/*if(hasTransInfo || !colorStr.CompareNoCase("HHHHHH") || true)*/
			if("" != colorInfo.line)
			{	
 				...
 				else
				{
					...
					//AreaPen.CreatePen(PS_SOLID, 1, RGB(255,0,0));
				}
				...
				int nParts = (int)pShape->m_aPartsStartingIndex.GetCount();
				...
				for(int i = 0; i < nParts; i++ )
				{
					...
					{	
						nPartEndIndex = (int)pShape->m_aPointArray.GetCount();
					}	 
					//if(nParts > 1)
					{
						pDC->MoveTo( round( pShape->m_aPointArray[nPartStartIndex].dX),
...
bool CFormatPaintTool::DrawLabel(CShape* pShape, ShapeUtility::ShapeType shapeType)
{
	...
	case ShapeUtility::point:
		{ 
			...
			int nFontHeight = (int)( this->nfontheight * dEnlargeRatio );//12
			int nFontWidth = this->nfontwidth;//8
			...
			CString sLable = pointLabel.ExtractPointLabel( pPointShape );
			...
			int nPictoWidth  = 8, nPictoHeight = 8;
			...
			for(int i = 0; i < nNumLetter; i++ )
			{
				...
				else if( isPutRight )
				{
					...
					else if( isGIFOutput )
					{
						pointLabelColor.DrawPointLabelShadow(pDC, posLetterRight.dX, posLetterRight.dY, sLetter);
						pDC->SetTextColor( pointLabelColor.GetGifPaletteColor(bgr(color)) );
						//pDC->SetTextColor( pointLabelColor.GetGifPaletteColor(color) );
					}
					else
					{
						pDC->SetTextColor( color );
	...
	case ShapeUtility::arc:
		{
			...
			if (m_pPlaceHolder->IsPlaceAvailable(letterPlaceList) && isFirstDegreeOffsetOK && isFirstDegreeOK)
			{
				isPutHalf = true;
			...
			else
			{
				...
				//////////////////////////////////////////////
				if (m_pPlaceHolder->IsPlaceAvailable(letterPlaceListInOneThird) && isSecondDegreeOffsetOK && isSecondDegreeOK)
				{
					isPutOneThird = true;
			...
			for(int i = 0; i < nNumLetter; i++ )
			{
				...
				VERIFY(font.CreateFont(
					...
					0,                         // nOrientation
					600,//FW_REGULAR,//FW_BOLD,//FW_NORMAL,//FW_BOLD,                 // nWeight
					FALSE,                     // bItalic
					FALSE,                     // bUnderline
					0,                         // cStrikeOut
					ANSI_CHARSET,              // nCharSet
					OUT_DEFAULT_PRECIS,        // nOutPrecision
					CLIP_DEFAULT_PRECIS,       // nClipPrecision
					ANTIALIASED_QUALITY,//PROOF_QUALITY,//DEFAULT_QUALITY,           // nQuality
					DEFAULT_PITCH | FF_SWISS,  // nPitchAndFamily
					//"Lucida Console"));    
					//"verdana"));
				    //"Arial"));
			...
			for(int i = 0; i < nNumLetter; i++ )
			{
				...
				if( isPutBottom )
				{
					...
					else if( isGIFOutput )
					{
						areaLabelColor.DrawAreaLabelShadow(pDC, posLetterBottom.dX, posLetterBottom.dY, sLetter);
						//pDC->SetTextColor( AreaColorManager.GetGifPaletteColor(bgr(color)) );
						pDC->SetTextColor( AreaColorManager.GetGifPaletteColor(color) );
						//pDC->SetTextColor(RGB(55,55,3));
					}
					else
					{

...
bool CDataBaseLayerDataSource::QuerySQLServer(std::string table, std::string field, bool isQueryFullyInsideTile)
{
	...
	if(dQuadTileInside_long2>dQuadTileInside_long1)
	{
		double temp;
		temp = dQuadTileInside_long1;
		dQuadTileInside_long1 = dQuadTileInside_long2;
		dQuadTileInside_long2 = temp;
	}
	if(dQuadTileInside_lat2>dQuadTileInside_lat1)
	{
		double temp;
		temp = dQuadTileInside_lat1;
		dQuadTileInside_lat1 = dQuadTileInside_lat2;
		dQuadTileInside_lat2 = temp;
	}
	...
	if( ShapeUtility::point == this->nLayerType )//for points
	{
		/*str_sql = "select " + field + ", " + table + ".geomValid.ToString() from " + str_database + ".dbo." + table 
			+ " where " + table + ".geomValid.STWithin(geometry::STGeomFromText('Polygon((" + polyString 
			+ "))',0))=1;";*/
	...
	else
	{
		...
		str_sql = "select * from " + str_database + ".dbo." + table 
			+ "_QuadTile_Inside where long1>=" + str_QuadTileInside_long2 
			+ " and lat1<="  + str_QuadTileInside_lat1
			+ " and long2<=" + str_QuadTileInside_long1
...
bool CDataBaseLayerDataSource::GetRecordFromSQLServer(CShapePtrArray& apShapes)
{
	...
	try
	{
		...
		else if(!this->prs_sqlserver->ADOEOF)
		{

...
public:	
	...
	{		
		...
		if( newtime->tm_mon+1 < 10 )
			sprintf( tempFileName, "c:\\logs\\%s%d0%d%d_test.log", obj.getLogFileName().c_str(), newtime->tm_year+1900, newtime->tm_mon+1, newtime->tm_mday ); 
			...
			sprintf( tempFileName, "c:\\logs\\%s%d%d%d_test.log", obj.getLogFileName().c_str(), newtime->tm_year+1900, newtime->tm_mon+1, newtime->tm_mday ); 

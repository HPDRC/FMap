...
public:	
	...
	{		
		...
		if( newtime->tm_mon+1 < 10 )
			sprintf( tempFileName, "c:\\logs_1\\%s%d0%d%d.log", obj.getLogFileName().c_str(), newtime->tm_year+1900, newtime->tm_mon+1, newtime->tm_mday ); 
			...
			sprintf( tempFileName, "c:\\logs_1\\%s%d%d%d.log", obj.getLogFileName().c_str(), newtime->tm_year+1900, newtime->tm_mon+1, newtime->tm_mday ); 

...
pair<double, double> Overlay2Qix::getDirName(double head, double tail, double dTokenizer)
{
	...
	//start = floor(head) - dTokenizer;
	//end   = floor(tail) + dTokenizer;
...
int Overlay2Qix::checkAndAddFile(vector<string>& files, char* path)
{
	...
	const int lenToken = 7;
	float tokenArray[lenToken] = {0.5, 1.0, 2.0, 4.0, 6.0, 8.0, 16.0};
	float offset = 0.5;
	//float tokenArray[lenToken] = {0.5, -0.5, 1.0, -1.0, 2.0, -2.0, 4.0, -4.0, 6.0, -6.0, 8.0, -8.0, 16.0, -16.0};

	int count = 0;

	for(int i=0; i<lenToken; i++)
	{
		//below codes is for testing
		/*if( 0 == i )
		{
		...
		/*pair<double, double> rangeLong = getDirName(longitude1, longitude2, 16);
		pair<double, double> rangeLat  = getDirName(latitude1,  latitude2,  16);*/
		...
		while(rangeLat.first <= rangeLat.second)
		{
			...
			rangeLat.first = rangeLat.first + dTokenizer;
			//rangeLat.first = rangeLat.first + offset;

	...
	if (stat == Gdiplus::Ok)
	{
		...
		for(long row = 0; row < dwHeight; row++)
		{
			for(long col = 0; col < dwWidth; col++)
			{
				...
				//		alpha_pixel) == true) )*/
				/*else if( (this->pColorTable->Lookup(colormanager.GetPNGColorTableIndex(pixelvalue), 
					alpha_pixel) == true) )
				{
					pixel = alpha_pixel;
				}*/

				//for street color only

...
COLORREF str2colorref(CString str, int type)
{
	...
	str = str.Trim();
	BYTE a = (BYTE) strtol((const char*)LPCTSTR(str.Left(2)), NULL, 16);
	BYTE r = (BYTE) strtol((const char*)LPCTSTR(str.Mid(2, 2)), NULL, 16);
	BYTE g = (BYTE) strtol((const char*)LPCTSTR(str.Mid(4, 2)), NULL, 16);
...
void CColorManager::SetArcColor(OverridedColor colorInfo)
{
	...
	if( "" == colorInfo.line)
	{
		...
		m_arccolor = color.ToCOLORREF();

	}
	else
	{
		BYTE a = (BYTE) strtol(colorInfo.line.substr(0,2).c_str(), NULL, 16);
		BYTE r = (BYTE) strtol(colorInfo.line.substr(2,2).c_str(), NULL, 16);
		BYTE g = (BYTE) strtol(colorInfo.line.substr(4,2).c_str(), NULL, 16);
		BYTE b = (BYTE) strtol(colorInfo.line.substr(6).c_str(), NULL, 16);

		//m_arccolor = bgr( RGB(r, g, b) );

		//Gdiplus::Color color(a, r, g, b);
		m_arccolor = RGB( r, g, b );
		//apply to alpha
		//m_arccolor |=  ( a << 24 );
...
void CColorManager::SetAreaColor(CString colorStr)
{
	...
	m_areacolor = str2colorref(colorStr, 0);// this is areacq
}
...
COLORREF CColorManager::AddShadow( COLORREF color )
{
	...
	BYTE r = (GetRValue(color) & ~0x03) | 0x03;
									 ...
									 bool hasDetailroads, bool hasMainroads)
{
	if(colorInfo.label != "")
	{
		BYTE r = (BYTE) strtol(colorInfo.label.substr(2,2).c_str(), NULL, 16);
		BYTE g = (BYTE) strtol(colorInfo.label.substr(4,2).c_str(), NULL, 16);
		BYTE b = (BYTE) strtol(colorInfo.label.substr(6).c_str(), NULL, 16);
	...
	{
		m_arclabelcolor = RGB(3,3,3);//green
	}
...
void CColorManager::SetAreaLabelColor(OverridedColor colorInfo, CString strAreaColor, int nColumnSize)
{
	...
	if(colorInfo.label != "")
	{
		...
		BYTE r = (BYTE) strtol(colorInfo.label.substr(0,2).c_str(), NULL, 16);
		BYTE g = (BYTE) strtol(colorInfo.label.substr(2,2).c_str(), NULL, 16);
		BYTE b = (BYTE) strtol(colorInfo.label.substr(4,2).c_str(), NULL, 16);

		//m_arealabelcolor = RGB(r,g,b);
		m_arealabelcolor = RGB(b,g,r);

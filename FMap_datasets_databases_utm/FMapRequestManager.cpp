...
bool CFMapRequestManager::hasFALayers(std::string overlay/*, bool& hasFAOwner, bool& hasFAADDR*/)
{
	bool hasFA = false;
	size_t foundfa;
	foundfa = overlay.find("FA_P");
	if(foundfa != string::npos)
	{
		hasFA = true;
	}
...
bool CFMapRequestManager::hasFAParcel(std::string overlay)
{
	bool hasFAParcel = false;
	size_t foundfa;
	foundfa = overlay.find("FA_Parcel");
	if(foundfa != string::npos)
	{
		hasFAParcel = true;
	}
...
void CFMapRequestManager::addFALayers(std::string overlay)
{
	falayers.push_back(overlay.c_str());
}

bool CFMapRequestManager::HasFilledAreaReq(std::string strAreaReq, bool hasFilledAreaReq/*, bool& hasFA*/)
{
	ASSERT(strAreaReq != "");
	...
	while(strOverlay != "")
	{
		...
		/*foundfa = strOverlay.find("FA_P");
		if(foundfa != string::npos)
		{
			falayers.push_back(strOverlay.c_str());
			hasFA = true;
		}*/
